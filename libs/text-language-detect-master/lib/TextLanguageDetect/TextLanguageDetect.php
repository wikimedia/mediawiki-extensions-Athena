...
use TextLanguageDetect\LanguageDetect\TextLanguageDetectParser;
...
class TextLanguageDetect
{
    ...
    private function _readdb($fname)
    {
        ...
        if (!file_exists($fname)) {
            throw new TextLanguageDetectException(
                'Language database does not exist: ' . $fname,
                TextLanguageDetectException::DB_NOT_FOUND
            );
        } elseif (!is_readable($fname)) {
            throw new TextLanguageDetectException(
                'Language database is not readable: ' . $fname,
                TextLanguageDetectException::DB_NOT_READABLE
            );
        }
        ...
        if (!is_array($trigram)) {
            if (ini_get('magic_quotes_runtime')) {
                throw new TextLanguageDetectException(
                    ...
                    TextLanguageDetectException::MAGIC_QUOTES
                );
            }
            throw new TextLanguageDetectException(
                'Language database is not an array.',
                TextLanguageDetectException::DB_NOT_ARRAY
            );
        } elseif (empty($trigram)) {
            throw new TextLanguageDetectException(
                'Language database has no elements.',
                TextLanguageDetectException::DB_EMPTY
            );
        }
    }
    ...
    public function languageExists($lang)
    {
        ...
        if (is_string($lang)) {
            ...
            throw new TextLanguageDetectException(
                ...
                TextLanguageDetectException::PARAM_TYPE
            );
        }
    }
    ...
    {
        // input check
        if (!TextLanguageDetectParser::validateString($sample)) {
            return array();
        }
        ...
        $sample_obj = new TextLanguageDetectParser($sample);
        ...
        if ($this->_use_unicode_narrowing) {
            ...
            if (is_array($blocks)) {
                ...
                throw new TextLanguageDetectException(
                    ...
                    TextLanguageDetectException::BLOCK_DETECTION
                );
            }
    ...
    public function detectUnicodeBlocks($str, $skip_symbols)
    {
        ...
        $sample_obj = new TextLanguageDetectParser($str);
        $sample_obj->prepareUnicode();
    ...
    public function clusterLanguages()
    {
        ...
        foreach ($langs as $lang) {
            if (!isset($this->_lang_db[$lang])) {
                throw new TextLanguageDetectException(
                    "missing $lang!",
                    TextLanguageDetectException::UNKNOWN_LANGUAGE
                );
            }
        }
        ...
        while (count($langs) > 2 && $i++ < 200) {
            ...
            if (!$highest_key1) {
                ...
                throw new TextLanguageDetectException(
                    ...
                    TextLanguageDetectException::NO_HIGHEST_KEY
                );
            }
    ...
    {
        // input check
        if (!TextLanguageDetectParser::validateString($str)) {
            return array();
        }
    ...
    private function _convertFromNameMode($lang, $convertKey = false)
    {
        ...
        if (is_string($lang)) {
            return (string)TextLanguageDetectISO639::$method($lang);
        }

        $newlang = array();
        foreach ($lang as $key => $val) {
            if ($convertKey) {
                $newkey = (string)TextLanguageDetectISO639::$method($key);
                $newlang[$newkey] = $val;
            } else {
                $newlang[$key] = (string)TextLanguageDetectISO639::$method($val);
            }
        }
        return $newlang;
    }
    ...
    private function _convertToNameMode($lang, $convertKey = false)
    {
        ...
        if (is_string($lang)) {
            return TextLanguageDetectISO639::$method($lang);
        }

        $newlang = array();
        foreach ($lang as $key => $val) {
            if ($convertKey) {
                $newkey = TextLanguageDetectISO639::$method($key);
                $newlang[$newkey] = $val;
            } else {
                $newlang[$key] = TextLanguageDetectISO639::$method($val);
            }
        }
        return $newlang;
    }
